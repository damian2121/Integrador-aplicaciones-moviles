$(function () {
    ...
    $('[name=buscar-filtro]').click(function () {
        var name = $('[name=name-filter]').val(),
            type = $('[name=type-filter]').prop('checked') ? 'track' : 'artist';
        ...
        .done(function (response) {
            ...
            if (type === 'track') {
                $.each(response.tracks.items, function (i, item) {
                    ...
                    agregarItem(item.name, urlImage, 'Escuchar', item.external_urls.spotify, item.id, 'add', item.uri);
                });
            } else if (type === 'artist') {
                $.each(response.artists.items, function (i, item) {
                    var urlImage = item.images.length === 0 ? null : item.images[0].url;
                    agregarItem(item.name, urlImage, 'Ver artista', item.external_urls.spotify, item.id, 'add');
                });
            }
        })
        .fail(function (error) {
