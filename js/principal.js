      ...
      if(item.images.length > 0){
        $(".prl-articles").append(
          $("<article/>").append( 
            ...
            $("<figcaption/>").text(item.name),
            $("<a/>").text("Ingresar a la Playlist").attr({target:"_blank", href: item.external_urls.spotify})
...
function tracksForPlayList(token, idPlayList){
  $.get({"url":"https://api.spotify.com/v1/playlists/"+idPlayList+"/tracks","headers":{"Authorization": "Bearer " + token}})
  .done(function (response) { 
    $.each(response.items, function(i, item) {
      var nameArtist = " - ", urlNameArtist = $("<li/>");
      ...
      $(".prl-articles").append(
        $("<li/>").append( 
          $("<a/>").text(item.tracks.name + nameArtist).attr({target:"_blank", href: item.tracks.external_urls})
